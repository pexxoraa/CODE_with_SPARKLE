...
class PortableProjectsTests(unittest.TestCase):
    ...
    def test_missing_legacy_project_is_ignored_during_migration(self):
        _, _, workspace, _ = self.old_project()
        shutil.rmtree(workspace.root)
        app = self.open_app()
        self.assertEqual(app.project(app.data["selected_project"])[1].root.parent, self.app_root / "PROJECTS")
        self.assertTrue((self.app_root / "PROJECTS").exists())
        self.assertTrue((self.app_root / "PROJECTS" / "my-project").exists())
