        ...
        with tempfile.TemporaryDirectory(prefix="sparkle-migration-", dir=bootstrap.parent) as staging:
            ...
            for project in data.get("projects", []):
                ...
                if not old.exists() or not old.is_dir():
                    continue  # A stale legacy entry can remain after the original folder was deleted.
                ...
                migrated_projects.append(project)
            data["projects"] = migrated_projects
            selected = data.get("selected_project")
            if selected and selected not in {project.get("id") for project in migrated_projects}:
